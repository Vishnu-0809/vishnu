...
class PurchaseHistoryFIlter5 extends StatefulWidget {
  const PurchaseHistoryFIlter5({
    super.key,
    // required this.
  ...
  @override
  State<PurchaseHistoryFIlter5> createState() => _PurchaseHistoryFIlter5State();
}

class _PurchaseHistoryFIlter5State extends State<PurchaseHistoryFIlter5> {
//  String link = "";
...
TextEditingController _controller = TextEditingController();
  ...
  //  Set<Details> newset=;
   ...
   List<Details> LastyearDetailsList = [];
...
void search(String query){
  ...
  final suggestions1=LastyearDetailsList.where((element){
...
//  print(difference);
  ...
  if((int.parse(temp.substring(0, 4)))<(int.parse(((DateTime.now().year)-3).toString())))
  {
    LastyearDetailsList.add(widget.DetailsList[i]);
  }
}
 SuggestionsDetailsList=LastyearDetailsList;
  ...
  if(SuggestionsDetailsList.length!=0)
  {
   ...
   var seen = Set<String>();
List<Details> uniquelist = SuggestionsDetailsList.where((student) => seen.add(student.details["brand"])).toList();
...
// showOverlay(context,"user not found");
          ...
          //    ScaffoldMessenger.of(context).showSnackBar(snackBar);
              ...
              // SuggestionsDetailsList.length!=0?
               Container(
                child: Container(
                  child:SuggestionsDetailsList.length==0? loading==true? Shimmer.fromColors(
